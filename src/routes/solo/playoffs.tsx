...
const incrementTurn = () => {
  ...
  if (turnIndex() > turn_order_light_cone_ban.length - 1) {
    return;
  }
  setTurnIndex(turnIndex() + 1);
  if (turnIndex() < turn_order_light_cone_ban.length) {
    setPlayerTurn(turn_order_light_cone_ban[turnIndex()].team);
  }
};
...
const handleReset = () => {
  ...
  setBlueCost(0);
  setRedCost(0);
  setPlayerTurn(turn_order_light_cone_ban[0].team);
...
const handleUndo = () => {
  ...
  decrementTurn();
  const currentTurn = turn_order_light_cone_ban[turnIndex()];
