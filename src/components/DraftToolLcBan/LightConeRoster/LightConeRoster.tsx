...
import { CharacterDetails, LCDetails } from "~/types";
import Results from "~/components/Results/Results";
import styles from "./Roster.module.css";

export interface LightConeRosterProps {
  handlePick: (character: CharacterPick) => void;
  handleBan: (character: CharacterBan) => void;
  handleReset: () => void;
  handleUndo: () => void;
}
const Roster: Component<LightConeRosterProps> = (props) => {
    ...
    <div class={styles.roster_container}>
      ...
      >
        ...
        <div class={styles.selector}>
          {Object.entries(lcJson())
            .sort(([a], [b]) => a.localeCompare(b))
            .map(([lcName, lcDetails]) => {
              const lcId = (lcDetails as LCDetails).id;
              const lcImage = `/light_cone_icons/${lcId}.webp`;
